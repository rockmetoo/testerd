...
void pkfin(){
	...
	int j;
	for(j=0; j<=16; j++) cle[j] = 0;
	for(j=0; j<=7; j++) x1a0[j] = 0;

	pkax = 0; pkbx = 0; pkcx = 0; pkdx = 0;
	pksi = 0; pktmp = 0; x1a2 = 0; pkres = 0;
	pki = 0; inter = 0; cfc = 0; cfd=0;
	compte = 0; pkc = 0;
}

void pkcode(){

	pkdx = x1a2+pki; pkax = x1a0[pki];
	pkcx = 0x015a; pkbx = 0x4e35;

	pktmp = pkax; pkax = pksi;
	pksi = pktmp; pktmp = pkax;
	pkax = pkdx; pkdx = pktmp;

	if(pkax != 0) pkax=pkax * pkbx;

	pktmp = pkax; pkax = pkcx;
	pkcx = pktmp;
	...
	pktmp = pkax; pkax = pksi; pksi = pktmp;
	pkax = pkax * pkbx; pkdx = pkcx+pkdx;
