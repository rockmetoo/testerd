...
void ping(struct sockaddr* ai_addr, int sock, uint16_t seq)
{
	...
	if(sendto(sock, &message, sizeof(message), 0, ai_addr, sizeof(*ai_addr)) < 0)
	{
		logError("sendto error [ping - %d]\n", __LINE__);
		return;
	}
}
...
struct sockaddr_in pong(int sock)
{
	...
	socklen_t fromlen = (socklen_t) sizeof(sendr);

	if(recvfrom(sock, &message, sizeof(message), 0, (struct sockaddr*) &sendr, &fromlen) < 0)
	{
		logError("recvfrom error [pong - %d]\n", __LINE__);
