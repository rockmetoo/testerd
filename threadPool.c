...
struct threadpool* threadpoolInit(int num_of_threads)
{
	...
	if((pool = malloc(sizeof(struct threadpool))) == NULL) {
		logError("threadpool struct memory allocation error");
		return NULL;
	}
	...
	if(pthread_mutex_init(&(pool->free_tasks_mutex),NULL)) {
		logError("threadpool pthread_mutex_init error");
		free(pool);
		return NULL;
	}
	...
	if((pool->thr_arr = malloc(sizeof(pthread_t) * num_of_threads)) == NULL) {
		logError("threadpool memory allocation error (num_of_threads)");
		free(pool);
		return NULL;
	}
	...
	for (pool->num_of_threads = 0; pool->num_of_threads < num_of_threads; (pool->num_of_threads)++) {
		if(pthread_create(&(pool->thr_arr[pool->num_of_threads]),NULL,worker_thr_routine,pool)) {
			logError("threadpool thread creation error");

			threadpoolFree(pool,0);
...
int threadpoolAddTask(struct threadpool *pool, void (*routine)(void*), void *data, int blocking)
{
	...
	if(pool == NULL) {
		logError("the threadpool received as argument is NULL");
		return -1;
	}

	if(pthread_mutex_lock(&(pool->free_tasks_mutex))) {
		logError("threadpoolAddTask pthread_mutex_lock");
		return -1;
	}
	...
	while (threadpool_queue_is_empty(&(pool->free_tasks_queue))) {
		if(!blocking) {
			...
			if(pthread_mutex_unlock(&(pool->free_tasks_mutex))) {
				logError("threadpoolAddTask pthread_mutex_unlock");
				return -1;
			}
		...
		if(pthread_cond_wait(&(pool->free_tasks_cond),&(pool->free_tasks_mutex))) {
			logError("threadpoolAddTask pthread_cond_wait");
			if(pthread_mutex_unlock(&(pool->free_tasks_mutex))) {
				logError("threadpoolAddTask pthread_mutex_unlock");
			}
	...
	if((task = (struct threadpool_task*)threadpool_queue_dequeue(&(pool->free_tasks_queue))) == NULL) {
		logError("Failed to obtain an empty task from the free tasks queue");
		if(pthread_mutex_unlock(&(pool->free_tasks_mutex))) {
			logError("threadpoolAddTask pthread_mutex_unlock");
		}
	...
	if(threadpool_queue_enqueue(&(pool->tasks_queue),task)) {
		logError("Failed to add a new task to the tasks queue");
		if(pthread_mutex_unlock(&(pool->mutex))) {
			logError("threadpoolAddTask pthread_mutex_unlock");
		}
		return -1;
	...
	if(threadpool_queue_getsize(&(pool->tasks_queue)) == 1) {
		...
		if(pthread_cond_broadcast(&(pool->cond))) {
			logError("threadpoolAddTask pthread_cond_broadcast");
			if(pthread_mutex_unlock(&(pool->mutex))) {
				logError("threadpoolAddTask pthread_mutex_unlock");
			}
...
{
	return threadpool_queue_is_empty(&(pool->tasks_queue));
}
